...
public class AuditLogDAOImpl extends DBContext implements AuditLogDAO {
    ...
    public StaffProcedureKpiDTO getStaffProcedureKpi(int userId, String filterDate) {
        ...
        String paymentStatusIn = shared.enums.PaymentStatus.sqlInClause();
        String sql = """
                ...
                + paymentStatusIn + """
                    ...
                    WHERE a.UserId = ?
                      AND (
                            a.EntityName IN (N'Thanh toÃƒÂ¡n', N'Payment')
                            OR UPPER(ISNULL(a.Reason, N'')) LIKE N'%THU LÃ¡Â»â€  PHÃƒÂ%'
                            OR UPPER(ISNULL(a.Reason, N'')) LIKE N'%THU PHI%'
                          )
                      AND (
                            UPPER(ISNULL(a.Action, N'')) IN (
                                N'INSERT', N'UPDATE', N'THÃƒÅ M', N'NHÃ¡ÂºÂ¬P', N'CÃ¡ÂºÂ¬P NHÃ¡ÂºÂ¬T'
                            )
                            OR UPPER(ISNULL(a.Reason, N'')) LIKE N'%THU LÃ¡Â»â€  PHÃƒÂ%'
                          )
                """;
