...
import util.examstaff.CandidatePhotoStorageUtil;
...
public class CandidatePhotoServiceImpl implements CandidatePhotoService {
    ...
    public void normalizePhotoPaths(String webRoot, List<ExamRegistrationDTO> queue) {
        util.examstaff.CandidatePhotoPathUtil.normalizeQueue(webRoot, queue);
    }
    ...
    public boolean resolveCapturedPhoto(String webRoot, ExamRegistrationDTO reg) {
        ...
        String normalized = util.examstaff.CandidatePhotoPathUtil.normalizePhotoUrl(webRoot, reg.getPhotoUrl());
        if (normalized != null) {
