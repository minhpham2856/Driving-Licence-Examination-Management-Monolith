...
import enums.ExamSectionProfiles;
...
public class ExaminerSessionContextServiceImpl implements ExaminerSessionContextService {
    ...
    public void refresh(HttpSession session, int examinerUserId) {
        ...
        List<ExaminerSlot> slots = controlService.getLoginEligibleAssignments(examinerUserId);
        if (slots == null || slots.isEmpty()) {
            session.setAttribute(ATTR_HAS_ACTIVE, Boolean.FALSE);
            session.setAttribute(ATTR_MESSAGE,
                    "Chưa có ca thi đang diễn ra. Vui lòng chờ cán bộ coi thi phân công và bắt đầu ca.");
            return;
        }

        ExaminerSlot slot = slots.get(0);
        SectionType sectionType = ExamSectionProfiles.resolveType(slot.getExamTypeName());
