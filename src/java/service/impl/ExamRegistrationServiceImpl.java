...
public class ExamRegistrationServiceImpl implements ExamRegistrationService {
    ...
    private CandidateEnrollmentDTO toEnrollmentDto(Candidate candidate, ExamEnrollment enrollment) {
    ...
    @Override
    public ServiceResult<Void> updateProfile(UpdateEnrollmentProfileCommand command) {
        if (command.getCandidateId() <= 0) {
            return ServiceResult.fail(ErrorType.VALIDATION_FAILED, "Thí sinh không hợp lệ.");
        }
        Candidate candidate = candidateDAO.getById(command.getCandidateId());
        if (candidate == null) {
    ...
    @Override
    public ServiceResult<Void> updateScores(UpdateEnrollmentScoresCommand command) {
        if (command.getCandidateId() <= 0) {
            return ServiceResult.fail(ErrorType.VALIDATION_FAILED, "Thí sinh không hợp lệ.");
        }
        if (command.getTheoryScore() != null) {
            boolean passed = "passed".equalsIgnoreCase(command.getTheoryResult());
            if (!examScoreService.upsertSectionScore(command.getCandidateId(), ExamSection.THEORY,
                    command.getTheoryScore(), passed)) {
                return ServiceResult.fail(ErrorType.PERSISTENCE_FAILED, "Không thể cập nhật điểm lý thuyết.");
            }
        }
        if (command.getPracticalScore() != null) {
            boolean passed = "passed".equalsIgnoreCase(command.getPracticalResult());
            if (!examScoreService.upsertSectionScore(command.getCandidateId(), ExamSection.LAYOUT,
                    command.getPracticalScore(), passed)) {
    ...
    @Override
    public ServiceResult<Void> updateAllocatedRoom(UpdateAllocatedRoomCommand command) {
        if (command.getCandidateId() <= 0 || command.getAreaId() <= 0) {
            return ServiceResult.fail(ErrorType.VALIDATION_FAILED, "Thông tin phân phòng không hợp lệ.");
        }
        ExamEnrollment enrollment = enrollmentDAO.getLatestByCandidateId(command.getCandidateId());
        if (enrollment == null) {
            return ServiceResult.fail(ErrorType.NOT_FOUND, "Không tìm thấy ghi danh của thí sinh.");
        }
        List<ExamDevice> devices = deviceDAO.getDevicesByAreaId(command.getAreaId());
        ...
        if (!enrollmentDAO.assignExamDevice(command.getCandidateId(), enrollment.getSessionId(), deviceId)) {
    ...
    @Override
    public ServiceResult<Void> insert(UploadRecordDTO dto) {
    ...
    @Override
    public ServiceResult<Void> updateRoadScore(UpdateRoadScoreCommand command) {
        if (command.getCandidateId() <= 0) {
            return ServiceResult.fail(ErrorType.VALIDATION_FAILED, "Thí sinh không hợp lệ.");
        }
        boolean passed = "passed".equalsIgnoreCase(command.getPassed());
        if (!examScoreService.upsertSectionScore(command.getCandidateId(), ExamSection.ROAD,
                command.getScore(), passed)) {
