...
public class SessionServiceImpl implements SessionService {
    ...
    @Override
    public ServiceResult<SessionStartDTO> startSession(int sessionId, int staffUserId) {
        SessionViewDTO examSession = buildSessionViewDTO(sessionId);
        if (examSession == null) {
        ...
        SessionStartDTO data = buildSessionStartDto(examSession.isMorningSession(), withArea);
        return ServiceResult.ok(data, "Bắt đầu ca thi thành công.");
    }

    @Override
    public ServiceResult<SessionStartDTO> endSession(int sessionId) {
        SessionViewDTO examSession = buildSessionViewDTO(sessionId);
        if (examSession == null) {
            return ServiceResult.fail(ErrorType.NOT_FOUND, "Không tìm thấy ca thi (SessionId=" + sessionId + ").");
        }
        ...
        SessionStartDTO data = buildSessionStartDto(examSession.isMorningSession(), 0);
    ...
    @Override
    public void clearActiveSessionId(int sessionId) {
        if (activeSessionId == sessionId) {
            activeSessionId = 0;
        }
    }
    ...
    @Override
    public SessionViewDTO getSessionById(int id) {
        return buildSessionViewDTO(id);
    }

    @Override
    public List<SessionViewDTO> getAllSessions() {
        return buildSessionViewDTOList(sessionDAO.findAllOrdered());
    }
    ...
    @Override
    public List<AssignmentDTO> getLoginEligibleAssignments(int examinerUserId) {
        return buildAssignmentDTOList(assignmentDAO.findInProgressByExaminerId(examinerUserId));
    }

    private SessionViewDTO buildSessionViewDTO(int sessionId) {
        return buildSessionViewDTO(sessionDAO.getById(sessionId));
    }

    private SessionViewDTO buildSessionViewDTO(Session session) {
        if (session == null) {
            return null;
        }
    ...
    private List<SessionViewDTO> buildSessionViewDTOList(List<Session> sessions) {
        List<SessionViewDTO> list = new ArrayList<>();
        if (sessions == null) {
            return list;
        }
        for (Session session : sessions) {
            SessionViewDTO dto = buildSessionViewDTO(session);
            if (dto != null) {
                list.add(dto);
            }
        }
