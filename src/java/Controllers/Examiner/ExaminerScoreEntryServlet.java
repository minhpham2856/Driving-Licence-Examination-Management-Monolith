    ...
    private boolean handleScoreEntryAction(HttpServletRequest request, HttpServletResponse response,
        ...
        switch (action) {
            ...
            case "markAbsentScore" -> {
                ...
                String nextAfterAbsent = examinerService.markAbsentInScoreEntry(sessionId, sbd, session);
                if (nextAfterAbsent != null && !nextAfterAbsent.isBlank()) {
                    redirect(response, request, "/views/examiner/score-entry?sbd=" + urlEncode(nextAfterAbsent) + "&absentDone=" + urlEncode(sbd));
                } else {
