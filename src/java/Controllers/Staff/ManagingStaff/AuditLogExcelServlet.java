...
import Models.User;
import Services.AuditLogExcelService;
import Services.Impl.AuditLogExcelServiceImpl;
...
public class AuditLogExcelServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        int total = auditDAO.countUserLogs(currentUser.getId(), keyword, action, startDate, endDate);
        List<AuditDTO> logs = auditDAO.searchUserLogsPaginated(
                currentUser.getId(), keyword, action, startDate, endDate, 1, Math.max(total, 1));
