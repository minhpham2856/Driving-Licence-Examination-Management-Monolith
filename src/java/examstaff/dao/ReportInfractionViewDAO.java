 ...
 * Gom OccurrenceCount theo ScoreDeduction.Reason, chỉ section TH.
 * <pre>
 ...
 * Dùng Db2ExamSchemaSql.PRACTICAL_SECTION_TYPES nhúng tường minh trong SQL —
 * không splice CSV runtime (Approach B).
 *
 * Triển khai mặc định:
 * examstaff.dao.impl.ReportInfractionViewDAOImpl.
 */
public interface ReportInfractionViewDAO {

    /**
     * Lấy top lỗi trừ điểm của phần thực hành trong một kỳ thi.
     * Thực thi SELECT TOP (?) nhóm theo Reason trên chuỗi JOIN
     * DeductionRecord → ScoreDeduction → ExamScore → ExamSection → ExamResult → ExamEnrollment;
     * lọc section thực hành và ExamId, tính count cùng tỉ lệ phần trăm.
     * @param examId mã kỳ thi (ExamEnrollment.ExamId) cần thống kê
     * @param limit  số dòng tối đa trả về; nếu ≤ 0 thì dùng mặc định (thường là 3)
     * @return danh sách Map mỗi phần tử gồm khóa reason, count,
     *         percentage; rỗng nếu kỳ thi không có lỗi trừ điểm
     */
