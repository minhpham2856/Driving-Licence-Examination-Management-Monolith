...
public class DocumentServiceImpl implements DocumentService {
    ...
    private static void writeCandidateSheet(Workbook wb, CellStyle headerStyle, CellStyle dateStyle,
        ...
        for (ExamRegistrationDTO reg : candidates) {
            ...
            if (reg.skipsTheory()) {
                ...
                r.createCell(col++).setCellValue(ExamStaffLabels.formatTheoryResult(reg));
            } else {
            ...
            if (reg.skipsPractical()) {
                r.createCell(col++).setBlank();
                r.createCell(col++).setCellValue(ExamStaffLabels.formatPracticalResult(reg));
            } else {
                writeScoreCell(r.createCell(col++), reg.getPracticalScore());
                r.createCell(col++).setCellValue(ExamStaffLabels.formatPracticalResult(reg));
            }
            String finalResult = ExamStaffLabels.formatFinalResult(reg);
            r.createCell(col++).setCellValue(finalResult);
            r.createCell(col++).setCellValue(ExamStaffLabels.yesNo(reg.isIsPaymentCompleted()));
            r.createCell(col++).setCellValue(ExamStaffLabels.yesNo(reg.isValidCapturedPhoto()));
    ...
    private void writeFeeSheet(Workbook wb, CellStyle headerStyle,
        ...
        int stt = 1;
        double grandTotal = 0;
        // Mutate — chỉ thí sinh đã thanh toán, không vắng
        for (ExamRegistrationDTO reg : candidates) {
