...
public class FeeDAOImpl extends DBContext implements FeeDAO {
    ...
    private boolean appliesToProcedure(Fee fee, boolean motorcycle, boolean requiresRoadTest) {
        ...
        if (name.contains("lââ thuyÃâât") || name.contains("ly thuyet")) {
            return true;
        }
        if (name.contains("sa hâÂ¼nh") || name.contains("sa hinh")) {
            return !motorcycle;
        }
        if (name.contains("âÃ¦ââÃâÂ¥ng trââÃâÂ¥ng") || name.contains("duong truong")) {
            return requiresRoadTest && !motorcycle;
        }
        if (name.contains("hÃâÃ´ sâÃ­") || name.contains("ho so") || "admin".equals(type)) {
            return true;
        }
