(function () {
    ...
    const SESSION_ID = parseInt(body.dataset.callSessionId || '0', 10);
    ...
    const I18N = {
        ...
        noSessionLabel: msg('msg-no-session-label', 'Ch\u01b0a ch\u1ecdn ca thi'),
        noSessionQueue: msg('msg-no-session-queue', 'Ch\u01b0a c\u00f3 ca thi \u2014 kh\u00f4ng hi\u1ec3n th\u1ecb danh s\u00e1ch ch\u1edd'),
        noSessionSync: msg('msg-no-session-sync', 'Ch\u01b0a k\u1ebft n\u1ed1i ca thi')
    };
    ...
    function renderNoSessionBoard() {
        if (sessionBadge) sessionBadge.textContent = I18N.noSessionLabel;
        if (queueList) queueList.innerHTML = '<li class="tv-queue-empty">' + escHtml(I18N.noSessionQueue) + '</li>';
        if (syncStatus) {
