-- ============================================================
-- DATABASE SCHEMA - DLEM_DB_2
-- Driving Licence Examination Management System
--
-- Scope bổ sung:
--   1. Managing Staff mở ngày thi dự kiến.
--   2. Ngày dự kiến được khóa trước hạn 07 ngày làm việc (do code xử lý).
--   3. Police Staff xem hồ sơ điện tử và duyệt/từ chối từng thí sinh.
--   4. Police Staff xuất danh sách cuối cùng.
--   5. Managing Staff import danh sách để tạo phiên thi chính thức.
--   6. Registrant xem lịch thi và kết quả thông qua ExamRegistrationId.
--
-- CẢNH BÁO: File này XÓA và TẠO LẠI DLEM_DB_2.
-- Không chạy trên database đang có dữ liệu cần giữ lại.
-- ============================================================
...
-- DocumentUrl lưu Cloudinary reference hoặc đường dẫn local.
-- Police Staff xem tài liệu qua Profile, không sao chép tài liệu sang bảng khác.
...
-- Hồ sơ đăng ký thi của Registrant.
...
-- Ngày thi dự kiến do Managing Staff tạo.
-- Status quản lý việc Registrant được chọn ngày hay không.
-- PoliceStatus quản lý việc Police đã tiếp nhận/xử lý danh sách hay chưa.
...
-- Theo nghiệp vụ hiện tại: một ngày chỉ mở một ngày thi dự kiến,
-- không phụ thuộc hạng GPLX.
CREATE UNIQUE INDEX UX_ExamDates_ExamDate
    ON ExamDates(ExamDate);
GO

-- Danh sách Registrant chọn ngày thi dự kiến.
...
-- Danh sách thi chính thức do Police Staff lập. ExamRegistrationId để NULL
-- đối với thí sinh đến từ đơn vị khác, vì họ không có tài khoản tại trung tâm.
...
-- Phiên thi chính thức do Managing Staff tạo sau khi Police hoàn tất duyệt.
-- SourceExamDateId cho biết phiên này được tạo từ ngày dự kiến nào.
CREATE TABLE Exam (
...
-- Dữ liệu kỳ thi lịch sử/import ngoài luồng có thể để SourceExamDateId = NULL.
...
-- Thí sinh ngày thi. Candidate vẫn tách biệt User/Profile để không ảnh hưởng
-- các module Exam Staff hiện tại.
...
-- ExamRegistrationId nối thí sinh trong phiên chính thức về tài khoản
-- Registrant để xem lịch thi, điểm và kết quả.
...
-- Cho phép NULL để tương thích dữ liệu lịch sử/dữ liệu test ngoài luồng Registrant.
...
-- Audit dùng để lưu ai gửi Police, ai duyệt/từ chối và thời điểm thao tác.
CREATE TABLE Audit (
...
PRINT N'Đã tạo DLEM_DB_2 với luồng Police Staff tối giản.';
GO
