...
<c:if test="${empty activeNav}">
    <c:choose>
        <c:when test="${fn:contains(pageContext.request.requestURI, 'home.jsp') or fn:contains(pageContext.request.requestURI, 'about') or pageContext.request.requestURI eq '/' or fn:endsWith(pageContext.request.requestURI, '/')}">
        ...
        <c:when test="${fn:contains(pageContext.request.requestURI, 'licenseTypes')}">
    ...
    <body class="${bodyClass}">
        <header class="top-nav-bar" role="banner" data-node-id="1:1221">
            <div class="top-nav-bar__container" data-node-id="1:1222">
                <a href="home.jsp" class="top-nav-bar__logo" aria-label="Lái Vui - Trang chủ" data-node-id="40:18">
                ...
                <nav class="top-nav-bar__nav" aria-label="Điều hướng chính" data-node-id="1:1225">
                    <a href="home.jsp"
                       class="top-nav-bar__link${activeNav eq 'gioi-thieu' ? ' is-active' : ''}"
                       data-node-id="1:1226"
                       <c:if test="${activeNav eq 'gioi-thieu'}">aria-current="page"</c:if>>
                           Giới thiệu
                       </a>
                       <a href="process.jsp"
                          class="top-nav-bar__link${activeNav eq 'quy-trinh' ? ' is-active' : ''}"
                       data-node-id="1:1228"
                       <c:if test="${activeNav eq 'quy-trinh'}">aria-current="page"</c:if>>
                           Quy trình
                       </a>
                       <a href="licenseTypes.jsp"
                          class="top-nav-bar__link${activeNav eq 'hang-bang' ? ' is-active' : ''}"
                       data-node-id="1:1230"
                       <c:if test="${activeNav eq 'hang-bang'}">aria-current="page"</c:if>>
                        Hạng bằng
                    </a>
                </nav>
