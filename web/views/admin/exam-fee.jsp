    ...
    <main class="main-content">
        ...
        <header class="page-header">
            <div class="page-title-wrap">
                ...
                <p class="page-subtitle">Thiết lập cấu hình đơn giá lệ phí sát hạch lý thuyết, thực hành sa hình, chi phí thuê xe chip và lệ phí cấp bằng cho từng hạng GPLX.</p>
            </div>
            <div class="page-actions" style="display: flex; gap: 10px;">
                ...
                <button class="btn-filter" id="btn-add-fee" style="height: 42px; padding: 0 1.25rem; font-size: 0.9rem; border-radius: 8px; flex: none;">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <!-- Stats Metrics Row -->
        <section class="metrics-row" aria-label="Thống kê lệ phí sát hạch">
            <div class="stat-card">
                ...
                <div class="filter-grid" style="grid-template-columns: 2fr 1.25fr 1.1fr 1.1fr 1.75fr;">
                    <div class="input-group">
                        ...
                        <input type="text" id="searchKeyword" name="searchKeyword" class="input-field"
                               placeholder="Nhập mã, tên biểu phí..." value="${param.searchKeyword}">
                    </div>
                    <div class="input-group">
                        ...
                        <select id="filterClass" name="filterClass" class="input-field">
                            ...
                            <c:forEach var="cl" items="${licenceClassesList}">
                                <option value="${cl.code}" ${param.filterClass eq cl.code ? 'selected' : ''}>Hạng ${cl.code}</option>
                            </c:forEach>
                        </select>
                    </div>
                    <div class="input-group">
                    ...
                    <div class="input-group filter-grid__btn-col">
                        <div class="btn-group">
                            ...
                            <a href="${pageContext.request.contextPath}/views/admin/exam-fee.jsp" class="btn-reset">Đặt lại</a>
                        </div>
                    </div>
                </div>
            </form>
        ...
        <section class="log-card" aria-label="Danh sách lệ phí thi">
                ...
                <table class="audit-table">
                    <thead>
                        <tr>
                            ...
                            <th scope="col">Tên gọi biểu phí sát hạch</th>
                            ...
                            <c:when test="${not empty examFees}">
                                <c:forEach var="fee" items="${examFees}" varStatus="status">
                                    <tr>
                                        ...
                                        <td style="text-align: center;">
                                            <c:choose>
                                                <c:when test="${fee.category eq 'theory'}">
                                                    <span class="role-badge role-badge--coi" style="color: #06b6d4; background-color: rgba(6,182,212,0.06); border-color: rgba(6,182,212,0.15);">Thi lý thuyết</span>
                                                </c:when>
                                                <c:when test="${fee.category eq 'practical'}">
                                                    <span class="role-badge role-badge--other" style="color: #b45309; background-color: rgba(245, 158, 11, 0.06); border-color: rgba(245, 158, 11, 0.15);">Thi sa hình</span>
                                                </c:when>
                                                <c:when test="${fee.category eq 'rent'}">
                                                    <span class="role-badge role-badge--coi" style="color: #7c3aed; background-color: rgba(124, 58, 237, 0.06); border-color: rgba(124, 58, 237, 0.15);">Thuê xe chip</span>
                                                </c:when>
                                                <c:otherwise>
                                                    <span class="role-badge role-badge--other" style="color: #475569; background-color: rgba(100, 116, 139, 0.06); border-color: rgba(100, 116, 139, 0.15);">Lệ phí cấp bằng</span>
                                                </c:otherwise>
                                            </c:choose>
                                        </td>
                                        ...
                                        <td style="text-align: center;">
                                            <c:choose>
                                                <c:when test="${fee.status eq 'active'}">
                                ...
                                <tr>
                                    <td colspan="9" style="text-align: center; padding: 5rem 1.5rem; color: #64748b; font-weight: 500;">
                                        ...
                                        <p style="font-size: 0.82rem; font-weight: 400; color: #94a3b8; margin-top: 0.5rem; max-width: 450px; margin-left: auto; margin-right: auto;">
                                            Nhấn nút <strong>Thêm cấu hình phí</strong> để khai báo đơn giá lệ phí thi lý thuyết, thực hành, thuê xe hoặc cấp bằng đầu tiên.
                                        </p>
                                    </td>
                                </tr>
                            </c:otherwise>
            ...
            <footer class="pagination-footer">
                <div class="pagination-info">
                    Hiển thị
                    <c:choose>
                        <c:when test="${not empty examFees}">1 - ${fn:length(examFees)}</c:when>
                        <c:otherwise>0</c:otherwise>
                    </c:choose>
                    trong tổng số
                    <c:choose>
                        <c:when test="${not empty totalFees}">${totalFees}</c:when>
                        <c:otherwise>0</c:otherwise>
                    </c:choose>
                    biểu phí thi
                </div>
