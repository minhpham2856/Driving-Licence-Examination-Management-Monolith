...
<html lang="vi">
    ...
    <body class="has-side-nav-bar">
                                                ...
                                                <td>
                                                    <div style="display: flex; gap: 5px; justify-content: center; flex-wrap: wrap;">
                                                        ...
                                                        <button type="button" class="btn-export"
                                                                ...
                                                                data-type="${fn:escapeXml(dev.deviceType)}" data-status="${dev.status}"
                                                                data-room="${dev.examRoomId}" data-area="${dev.examAreaId}"
        ...
        <%-- Add/Edit modal --%>
        <div id="devModal" class="modal-overlay" onclick="if (event.target === this)
                    closeDevModal()">
            <div class="modal-card" role="dialog" aria-modal="true">
                <form id="devForm" action="${ctx}/admin/exam-computer?action=save" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div id="devFormError" style="display:none; margin-bottom:1.1rem; padding:0.7rem 0.9rem; border-radius:10px; font-weight:600; font-size:0.85rem; background:rgba(239,68,68,0.08); border:1px solid rgba(239,68,68,0.25); color:#b91c1c;"></div>

                        <%-- Bước 1: khu vực --%>
                        <div class="input-group" style="margin-bottom:1.25rem;">
                            <label for="m_area" class="input-label">Khu vực thi <span style="color:#dc2626;">*</span></label>
                            <select id="m_area" name="examAreaId" class="input-field" required onchange="onDevAreaChange()">
                        ...
                        <%-- Bước 2: phòng thi (ẩn đến khi chọn khu vực) --%>
                        <div class="input-group" id="devRoomWrap" style="display:none; margin-bottom:1.25rem;">
                            <label for="m_room" class="input-label">Phòng thi <span style="color:#dc2626;">*</span></label>
                            <select id="m_room" name="examRoomId" class="input-field" onchange="onDevRoomChange()">
                                <option value="">-- Chọn phòng thi --</option>
                            </select>
                        </div>

                        <%-- Bước 3: các field còn lại (ẩn đến khi chọn phòng) --%>
                        <div id="devFields" style="display:none;">
                            <div class="input-group" style="margin-bottom:1.25rem;">
                                ...
                                <input type="text" id="m_name" name="deviceName" class="input-field" placeholder="VD: Máy trạm thi số 01">
                            </div>
                            <div class="filter-grid" style="grid-template-columns:1fr 1fr; gap:1.25rem;">
                                <div class="input-group">
                                    ...
                                    <input type="text" id="m_type" name="deviceType" class="input-field" list="deviceTypeList"
                                           placeholder="VD: Xe gắn máy, Xe ô tô...">
                                ...
                                <div class="input-group">
                                    ...
                                    <select id="m_status" name="status" class="input-field">
        ...
        <%-- Detail modal --%>
        <div id="devDetail" class="modal-overlay" onclick="if (event.target === this)
                    closeDevDetail()">
        ...
        <script>
            const DEV_ROOM_URL = '${ctx}/admin/exam-computer?action=roomsByArea';
            ...
            function showDevError(m) {
                var e = document.getElementById('devFormError');
                e.textContent = m;
            ...
            function resetDevModal(title) {
                document.getElementById('devModalTitle').textContent = title;
                ['m_id', 'm_name', 'm_type', 'm_status', 'm_area'].forEach(function (k) {
                    document.getElementById(k).value = '';
                });
                document.getElementById('m_room').innerHTML = '<option value="">-- Chọn phòng thi --</option>';
                document.getElementById('devRoomWrap').style.display = 'none';
                document.getElementById('devFields').style.display = 'none';
                hideDevError();
            }

            function loadDevRooms(areaId, cb) {
                var wrap = document.getElementById('devRoomWrap');
                var sel = document.getElementById('m_room');
                if (!areaId) {
                ...
                fetch(DEV_ROOM_URL + '&areaId=' + encodeURIComponent(areaId))
                        .then(function (r) {
                            return r.json();
                        })
                        .then(function (rooms) {
                            var html;
                            if (!rooms.length) {
                                html = '<option value="">(Khu vực này chưa có phòng thi)</option>';
                            } else {
                                html = '<option value="">-- Chọn phòng thi --</option>';
                                rooms.forEach(function (r) {
                                    html += '<option value="' + r.id + '">' + escapeHtml(r.code + ' · ' + r.name + ' (' + r.typeLabel + ')') + '</option>';
                                });
                            }
                            sel.innerHTML = html;
                        ...
                        .catch(function () {
                            sel.innerHTML = '<option value="">Không tải được phòng</option>';
                        });
            }
            ...
            function onDevAreaChange() {
                hideDevError();
                document.getElementById('devFields').style.display = 'none';
                loadDevRooms(document.getElementById('m_area').value, null);
            }
            function onDevRoomChange() {
                document.getElementById('devFields').style.display = document.getElementById('m_room').value ? 'block' : 'none';
            }

            function openDevModal() {
                resetDevModal('Thêm máy thi');
                document.getElementById('devModal').classList.add('is-open');
            }
            function openDevModalEdit(b) {
                resetDevModal('Chỉnh sửa máy thi');
                document.getElementById('m_id').value = b.dataset.id;
                document.getElementById('m_area').value = b.dataset.area;
                loadDevRooms(b.dataset.area, function () {
            ...
            document.getElementById('devForm').addEventListener('submit', function (e) {
                ...
                fetch(form.action, {method: 'POST', headers: {'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'}, body: body})
                        ...
                        .then(function (res) {
                            if (res.ok) {
                                window.location.href = '${ctx}/admin/exam-computer';
                            } else {
                                showDevError(res.message || 'Đã xảy ra lỗi.');
                            }
                        })
                        .catch(function () {
                            showDevError('Không gửi được yêu cầu. Vui lòng thử lại.');
                        });
            });
            ...
            function openDevDetail(b) {
                ...
                var stLabel = (st === 'Operational' || st === 'Available') ? 'Đang hoạt động' : (st === 'Maintenance' ? 'Đang bảo trì' : 'Hỏng / Khóa');
            ...
            function closeDevDetail() {
                document.getElementById('devDetail').classList.remove('is-open');
            }
