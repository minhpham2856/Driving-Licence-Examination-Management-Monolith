    ...
    <main class="main-content">
        ...
        <header class="page-header">
            <div class="page-title-wrap">
                ...
                <p class="page-subtitle">Theo dõi toàn bộ thao tác của người dùng trên hệ thống: đăng nhập, thay đổi dữ liệu, phân quyền và các cảnh báo bất thường.</p>
            </div>
        </header>
        ...
        <section class="metrics-row" aria-label="Thống kê nhật ký">
            <div class="stat-card">
                ...
                <div class="stat-info">
                    <span class="stat-number">${empty totalActions ? 0 : totalActions}</span>
            ...
            <div class="stat-card">
                ...
                <div class="stat-info">
                    <span class="stat-number">${empty updateCount ? 0 : updateCount}</span>
                    <span class="stat-label">Thao tác cập nhật</span>
                    <span class="stat-trend stat-trend--up">Thay đổi dữ liệu</span>
                </div>
            </div>
            <div class="stat-card">
                ...
                <div class="stat-info">
                    <span class="stat-number">${empty warningCount ? 0 : warningCount}</span>
                    <span class="stat-label font-bold">Cảnh báo / sự cố</span>
                    <span class="stat-trend stat-trend--down">Cần kiểm tra</span>
                </div>
            </div>
            <div class="stat-card">
                ...
                <div class="stat-info">
                    <span class="stat-number">${empty successRate ? '100.0' : successRate}%</span>
                    <span class="stat-label">Tỷ lệ tác vụ bình thường</span>
                ...
                <div class="filter-grid" style="grid-template-columns: 2fr 1.2fr 1.2fr 1fr 1fr 1.4fr;">
                    <div class="input-group">
                        ...
                        <input type="text" id="searchKeyword" name="searchKeyword" class="input-field"
                               placeholder="Tìm tài khoản, hành động, nội dung..." value="${param.searchKeyword}">
                    </div>
                    <div class="input-group">
                    ...
                    <div class="input-group">
                        <label for="dateFrom" class="input-label">Từ ngày</label>
                        <input type="date" id="dateFrom" name="dateFrom" class="input-field" value="${param.dateFrom}">
                    </div>
                    <div class="input-group">
                        <label for="dateTo" class="input-label">Đến ngày</label>
                        <input type="date" id="dateTo" name="dateTo" class="input-field" value="${param.dateTo}">
                    </div>
        ...
        <section class="log-card" aria-label="Sổ hoạt động nhật ký kiểm toán">
                ...
                <h2 class="log-card-title">
                    ...
                    <c:if test="${not empty auditLogs}">
                        <span style="font-size: 0.78rem; font-weight: 600; background: rgba(0,82,204,0.08); color: #0052cc; padding: 2px 10px; border-radius: 9999px; margin-left: 6px;">
                            ${empty filteredTotal ? fn:length(auditLogs) : filteredTotal} bản ghi
                        </span>
                    </c:if>
                </h2>
                            ...
                            <c:when test="${not empty auditLogs}">
                                <c:forEach var="log" items="${auditLogs}" varStatus="status">
                                    <tr>
                                        ...
                                        <td style="font-size: 0.82rem; color: #475569; font-weight: 500;">
                                        ...
                                        <td>
                                            <div class="user-cell">
                                                ...
                                                <div class="user-info">
                                                    <span class="user-name" style="font-weight: 600; color: #0f172a;">${log.displayName}</span>
                                                    <c:if test="${not empty log.username}"><span class="user-username">@${log.username}</span></c:if>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td style="text-align: center;">
                                            <span class="user-username" style="font-weight: 600;">${log.module}</span>
                                        </td>
                                        <td style="font-size: 0.85rem; color: #334155; white-space: normal;">
                                            <c:choose>
                                                <c:when test="${log.action eq 'WARNING'}">
                                                    <span style="color:#b91c1c; font-weight:600;">${log.detail}</span>
                                                </c:when>
            ...
            <footer class="pagination-footer">
                ...
                <div class="pagination-info">
                    Hiển thị
                    <c:choose>
                        <c:when test="${not empty auditLogs}">${(currentPage-1)*15 + 1} - ${(currentPage-1)*15 + fn:length(auditLogs)}</c:when>
                        <c:otherwise>0</c:otherwise>
                    </c:choose>
                    trong tổng số ${empty filteredTotal ? 0 : filteredTotal} bản ghi kiểm toán
                </div>
                <div class="pagination-nav">
                    <c:choose>
                        <c:when test="${currentPage > 1}">
                            <a class="page-btn page-btn--wide" href="${ctx}/admin/audit?${qs}&page=${currentPage-1}">Trước</a>
                        </c:when>
                        <c:otherwise><button class="page-btn page-btn--wide disabled" disabled>Trước</button></c:otherwise>
                    </c:choose>
                    <button class="page-btn active">${empty currentPage ? 1 : currentPage}</button>
                    <c:choose>
                        <c:when test="${currentPage < totalPages}">
                            <a class="page-btn page-btn--wide" href="${ctx}/admin/audit?${qs}&page=${currentPage+1}">Sau</a>
                        </c:when>
                        <c:otherwise><button class="page-btn page-btn--wide disabled" disabled>Sau</button></c:otherwise>
                    </c:choose>
                </div>
            </footer>
        </section>
