...
<div class="table-responsive"><table class="audit-table"><thead><tr><th>Phiên thi</th><th>Ngày thi</th><th>Hạng</th><th>Trung tâm</th><th>Khu vực</th><th>Thí sinh</th><th></th></tr></thead><tbody>
<c:forEach var="exam" items="${upcomingSessions}"><tr><td><strong><c:out value="${exam.sessionName}" /></strong></td><td><fmt:formatDate value="${exam.examDate}" pattern="dd/MM/yyyy" /></td><td>Hạng <c:out value="${exam.licenseCode}" /></td><td><c:out value="${exam.centreName}" /></td><td><c:out value="${exam.areaName}" /></td><td><strong>${exam.registeredCount}/${exam.maxCandidates}</strong><c:if test="${exam.registeredCount eq 0}"><br><small style="color:#dc2626">Chưa import danh sách</small></c:if></td><td><a class="btn-export" href="${ctx}/manager/exam-schedules?view=${exam.id}" style="text-decoration:none">Chi tiết</a></td></tr></c:forEach>
<c:if test="${empty upcomingSessions}"><tr><td colspan="7" style="padding:2rem;text-align:center;color:#64748b">Chưa có phiên thi sắp tới.</td></tr></c:if>
</tbody></table></div></section>
