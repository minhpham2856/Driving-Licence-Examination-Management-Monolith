...
<div class="dashboard-shell">
    <main class="main-content">
        ...
        <jsp:include page="/views/layout/header-examstaff.jsp">
            <jsp:param name="pageTitle" value="Tổng quan ca thi" />
            <jsp:param name="sectionTitle" value="Tổng quan ca thi" />
        </jsp:include>

        <nav class="breadcrumbs">
            <a href="${ctx}/views/public/home.jsp">Trang chủ</a>
            <span class="breadcrumbs__separator">/</span>
            <span class="breadcrumbs__current">Tổng quan ca thi</span>
        </nav>
            ...
            <div class="page-actions" style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
                <form action="${ctx}/views/staff/examstaff/dashboard" method="GET" style="margin: 0; display: inline-flex; align-items: center;">
                    <div style="background: rgba(255, 255, 255, 0.7); border: 1px solid rgba(226, 232, 240, 0.8); border-radius: 8px; padding: 4px 10px; display: flex; align-items: center; gap: 8px; height: 42px;">
                        ...
                        <select id="sessionId" name="sessionId" class="es-session-selector__select" onchange="this.form.submit()">
                            <c:forEach var="sess" items="${allSessions}">
                                <option value="${sess.id}" ${selectedSessionId eq sess.id ? "selected" : ""}>
                                    Ca #${sess.id} - ${sess.sessionLabel} (${sess.licenseCode})
                                </option>
                            </c:forEach>
                        </select>
                    </div>
                ...
                <a href="${ctx}/views/staff/examstaff/allocation?sessionId=${selectedSessionId}" class="btn-export" style="height: 42px; padding: 0 1.25rem; font-size: 0.9rem; border-radius: 8px; background-color: #ffffff; color: #475569; border-color: #e2e8f0; text-decoration: none; display: inline-flex; align-items: center; gap: 6px;">
                    Phân bổ khu vực
                </a>
            </div>
        </header>

        <c:if test="${not empty sessionControlMsg}">
            <div style="background-color: #ecfdf5; border: 1.5px solid #10b981; border-radius: 12px; padding: 0.88rem 1.25rem; margin-bottom: 1.25rem; display: flex; gap: 8px; align-items: center;">
                <span style="font-size: 0.85rem; font-weight: 700; color: #047857;">${sessionControlMsg}</span>
            </div>
        </c:if>
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem;">
                <div>
                    ...
                    <p style="font-size: 0.82rem; color: #64748b; margin: 0;">
                        ...
                        (<a href="${ctx}/views/staff/examstaff/examiner-allocation?sessionId=${selectedSessionId}" style="color: #0052cc; font-weight: 700;">Phân bổ giám khảo</a>).
                        Hiện có <strong>${assignedExaminerCount}</strong> giám khảo đã phân phòng.
                    </p>
                </div>
                <div style="display: flex; gap: 10px; flex-wrap: wrap; align-items: center;">
                    <c:choose>
                        <c:when test="${sessionStatusName eq "IN_PROGRESS"}">
                            <span class="role-badge role-badge--admin" style="background: #dcfce7; color: #166534; border: 1px solid #86efac;">${sessionStatusValue}</span>
                            <form action="${ctx}/staff/examstaff/session-control" method="POST" style="margin: 0;">
                        ...
                        <c:when test="${sessionStatusName eq "COMPLETED" or sessionStatusName eq "CANCELLED"}">
                            <span class="role-badge" style="background: #f1f5f9; color: #64748b;">${sessionStatusValue}</span>
                        </c:when>
                        <c:otherwise>
                            <span class="role-badge role-badge--coi" style="background: #fffbeb; color: #b45309; border: 1px solid #fde68a;">${sessionStatusValue}</span>
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <a href="${ctx}/views/staff/examstaff/examiner-allocation?sessionId=${selectedSessionId}" class="btn-filter" style="height: 40px; padding: 0 1.25rem; border-radius: 8px; font-weight: 700; text-decoration: none; display: inline-flex; align-items: center; opacity: 0.85;">
            ...
            <c:choose>
                <c:when test="${not empty currentSession}">
                    <ul style="list-style: none; padding: 0; margin: 0; display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
                        <li><span style="display:block; font-size: 0.75rem; color: #64748b; text-transform: uppercase; letter-spacing: 0.03em;">Ca sát hạch</span><strong style="color:#0f172a;">Ca #${currentSession.id} - ${currentSession.sessionLabel}</strong></li>
                        <li><span style="display:block; font-size: 0.75rem; color: #64748b; text-transform: uppercase; letter-spacing: 0.03em;">Hạng</span><strong style="color:#0f172a;">${currentSession.licenseCode}</strong></li>
                        <li><span style="display:block; font-size: 0.75rem; color: #64748b; text-transform: uppercase; letter-spacing: 0.03em;">Ngày thi</span><strong style="color:#0f172a;"><fmt:formatDate value="${currentSession.examDate}" pattern="dd/MM/yyyy" /></strong></li>
                        <li><span style="display:block; font-size: 0.75rem; color: #64748b; text-transform: uppercase; letter-spacing: 0.03em;">Thí sinh đăng ký</span><strong style="color:#0f172a;">${currentSession.registeredCount}</strong></li>
                        <li><span style="display: block; font-size: 0.75rem; color: #64748b; text-transform: uppercase; letter-spacing: 0.03em;">Trạng thái</span><strong style="color:#0f172a;">${sessionStatusValue}</strong></li>
                    </ul>
                </c:when>
                <c:otherwise>
                    <p style="color: #64748b; margin: 0;">Không tìm thấy ca thi cho mã đã chọn.</p>
                </c:otherwise>
            </c:choose>
        </section>
