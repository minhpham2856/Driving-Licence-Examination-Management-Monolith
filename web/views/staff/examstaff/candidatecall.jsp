...
<div class="dashboard-shell">
    <main class="main-content">
        ...
        <jsp:include page="/views/layout/header-examstaff.jsp">
            ...
            <jsp:param name="pageTitle" value="Gọi thí sinh vào làm thủ tục" />
        </jsp:include>
                ...
                <c:choose>
                    <c:when test="${sessionScope.shiftEnded eq 'true'}">
                        ...
                        <div class="waiting-list-pane" style="text-align: center; padding: 3rem 1.5rem; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 12px;">
                            ...
                            <a href="candidate-call?action=startShift" class="btn-batch" style="background: linear-gradient(135deg, #0052cc, #003d9b); border: none; font-size: 0.88rem; height: 42px; margin-top: 1rem; width: auto; padding: 0 1.5rem;">
                                ...
                                <c:when test="${empty sessionScope.callingSbd}">
                                    ...
                                    <c:choose>
                                        ...
                                        <c:otherwise>
                                            ...
                                            <a href="candidate-call?action=startCall" class="btn-batch" style="background: linear-gradient(135deg, #10b981, #059669); border: none; font-size: 0.92rem; height: 46px;">
                                    ...
                                    <a href="candidate-call?action=endShift" class="btn-batch btn-batch--alt" style="margin-top: 0.75rem; border-color: rgba(239, 68, 68, 0.2); color: #ef4444; background: rgba(239, 68, 68, 0.01); font-size: 0.85rem; height: 38px;">
                                ...
                                <c:otherwise>
                                    ...
                                    <div class="soundwave-container" style="margin-bottom: 0px;">
                                        ...
                                        <span style="color: #ffffff; font-size: 0.95rem; font-weight: 700; line-height: 1.4; max-width: 280px; display: block;">
                                            "Mời thí sinh có số báo danh ${callingCandidate.candidateNumber}, ${callingCandidate.fullName} nhanh chóng di chuyển vào bàn làm thủ tục!"
                                        </span>
                                    ...
                                    <div class="active-calling-card">
                                        ...
                                        <div style="font-family: monospace; font-size: 2.25rem; font-weight: 900; color: #0f172a; letter-spacing: 0.02em; line-height: 1.1;">
                                            ${callingCandidate.candidateNumber}
                                        </div>
                                        ...
                                        <div style="display: flex; flex-direction: column; gap: 8px; margin-top: 1.25rem; border-top: 1px solid #e2e8f0; padding-top: 1rem;">
                                            <a href="procedure?sbd=${callingCandidate.candidateNumber}#procedure-desk" class="btn-batch" style="background-color: #0052cc; border-color: #0052cc; height: 40px; font-size: 0.85rem;">
                                                Tiến hành lập hồ sơ &rarr;
                                            </a>

                                            <div style="display: flex; gap: 8px; width: 100%;">
                                                <a href="candidate-call?action=absent&sbd=${callingCandidate.candidateNumber}" class="btn-batch btn-batch--alt" style="flex: 1; height: 38px; border-color: rgba(245, 158, 11, 0.3); color: #d97706; background: rgba(245, 158, 11, 0.01); font-size: 0.8rem;" title="Đẩy xuống cuối hàng đợi">
                                                    Vắng tạm thời
                                                </a>
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.25rem; border-bottom: 1px solid #f1f5f9; padding-bottom: 0.75rem;">
                    ...
                    <c:if test="${sessionScope.shiftEnded ne 'true'}">
                        <div style="display: flex; gap: 6px; align-items: center;">
                            ...
                            <a href="candidate-call" class="btn-batch btn-batch--alt" style="width: 32px; height: 32px; padding: 0; display: inline-flex; align-items: center; justify-content: center; border-radius: 6px;" title="Làm mới hàng đợi">
                            ...
                            <table class="audit-table" style="font-size: 0.85rem; width: 100%;">
                                ...
                                <tbody>
                                    <c:forEach var="candidate" items="${sessionScope.candidateQueue}" varStatus="status">
                                        ...
                                        <c:if test="${not cDone}">
                                            <c:set var="isCurrentCalling" value="${candidate.candidateNumber eq sessionScope.callingSbd}" />
                                            ...
                                            <tr style="${isCurrentCalling ? 'background-color: rgba(0, 82, 204, 0.04); border-left: 3px solid #0052cc;' : ''}">
                                                ...
                                                <td style="text-align: right;">
                                                    <div style="display: inline-flex; gap: 4px;">
                                                        <a href="procedure?sbd=${candidate.candidateNumber}#procedure-desk" class="btn-filter" style="height: 26px; padding: 0 8px; font-size: 0.7rem; border-radius: 6px; text-decoration: none; display: inline-flex; align-items: center;">Hồ sơ</a>
                                                        <a href="candidate-call?action=absent&sbd=${candidate.candidateNumber}" class="btn-reset" style="height: 26px; padding: 0 8px; font-size: 0.7rem; border-radius: 6px; text-decoration: none; display: inline-flex; align-items: center; color: #d97706; border-color: rgba(245, 158, 11, 0.3); background: rgba(245, 158, 11, 0.02);" title="Đẩy xuống cuối hàng chờ">Vắng tạm</a>
                                                    </div>
                                                </td>
                                            </tr>
                                        </c:if>
        ...
        <!-- Permanent Absents List & Undo Option (Exception Safety) -->
                        ...
                        <tbody>
                            <c:forEach var="c" items="${sessionScope.permanentAbsents}">
                                <tr>
                                    ...
                                    <td style="text-align: right;">
                                        <a href="candidate-call?action=undoAbsent&sbd=${c.candidateNumber}" class="btn-filter" style="height: 26px; padding: 0 10px; font-size: 0.72rem; border-radius: 6px; text-decoration: none; display: inline-flex; align-items: center; border-color: #10b981; color: #10b981; background: rgba(16, 185, 129, 0.02);" title="Khôi phục về hàng đợi ban đầu">
                                            Hoàn tác
                                        </a>
                                    </td>
                                </tr>
...
<div id="candidateCallConfig"
     data-sbd="${not empty callingCandidate ? callingCandidate.candidateNumber : ''}"
     hidden></div>
